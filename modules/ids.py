...
def get_status_tag_ids(environment: str) -> list[str]:
    if environment == "STAGE":
        return [STAGE_CREATED_TAG, STAGE_INVITED_TAG, STAGE_ACCESSED_TAG]
    elif environment == "PROD":
        return [PROD_CREATED_TAG, PROD_INVITED_TAG, PROD_ACCESSED_TAG]
    else:
        raise ValueError(f"Invalid environment: {environment}")
...
def get_report_type_tag_id(environment: str, report_type: str) -> str:
    if environment == "STAGE":
        ...
        if report_type == "MGP":
            ...
            return PROD_MGS_REPORT_TYPE_TAG
        elif report_type == "MGPH":
            return PROD_MGPH_REPORT_TYPE_TAG
        elif report_type == "StandardMG":
...
def get_report_type_tag_ids(environment: str) -> list[str]:
    if environment == "STAGE":
        return [
            STAGE_MGP_REPORT_TYPE_TAG,
            STAGE_MGPH_REPORT_TYPE_TAG,
            STAGE_STD_MG_REPORT_TYPE_TAG,
        ]
    elif environment == "PROD":
        return [
            PROD_MGP_REPORT_TYPE_TAG,
            PROD_MGPH_REPORT_TYPE_TAG,
            PROD_STD_MG_REPORT_TYPE_TAG,
        ]
    else:
        raise ValueError(f"Invalid environment: {environment}")
...
def get_breast_density_tag_ids(environment: str) -> list[str]:
    if environment == "STAGE":
        return [
            STAGE_DENSE_BREAST_DENSITY_TAG,
            STAGE_NON_DENSE_BREAST_DENSITY_TAG,
        ]
    elif environment == "PROD":
        return [
            PROD_DENSE_BREAST_DENSITY_TAG,
            PROD_NON_DENSE_BREAST_DENSITY_TAG,
        ]
    else:
        raise ValueError(f"Invalid environment: {environment}")
...
def get_alr_tag_ids(environment: str) -> list[str]:
    if environment == "STAGE":
        return [
            STAGE_HIGH_LIFETIME_RISK_TAG,
            STAGE_INTERMEDIATE_LIFETIME_RISK_TAG,
            STAGE_AVERAGE_LIFETIME_RISK_TAG,
        ]
    elif environment == "PROD":
        return [
            PROD_HIGH_LIFETIME_RISK_TAG,
            PROD_INTERMEDIATE_LIFETIME_RISK_TAG,
            PROD_AVERAGE_LIFETIME_RISK_TAG,
        ]
    else:
        raise ValueError(f"Invalid environment: {environment}")
...
def get_birad_tag_ids(environment: str) -> list[str]:
    if environment == "STAGE":
        return [
            STAGE_BIRAD_0_TAG,
            STAGE_BIRAD_1_TAG,
...
def get_bac_tag_ids(environment: str) -> list[str]:
    if environment == "STAGE":
        return [STAGE_BAC_DETECTED_TAG, STAGE_BAC_NOT_DETECTED_TAG]
    elif environment == "PROD":
        return [PROD_BAC_DETECTED_TAG, PROD_BAC_NOT_DETECTED_TAG]
    else:
        raise ValueError(f"Invalid environment: {environment}")
