...
def main():
    ...
    for provider_id in PROD_PROVIDERS:
        ...
        while more_to_get:
            ...
            variables = {
                # "org_chat": True,
                "provider_filter ": str(provider_id),
                "offset": offset,
            }

            response = H.list_orders(variables)

            lab_orders = response["labOrders"]

            all_orders.extend(lab_orders)
            print(
                f"Fetched {len(lab_orders)} lab orders at offset {offset}. Total so far: {len(all_orders)}"
            )

            if not lab_orders:
                print("No lab orders to fetch. Loop complete.")
                more_to_get = False
